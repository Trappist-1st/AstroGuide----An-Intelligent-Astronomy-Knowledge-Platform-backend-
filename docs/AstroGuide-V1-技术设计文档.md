...
> **当前实现说明**：V1 固定管线已被 LangGraph Agent Runtime（Phase 3）替代。RAG 检索保留为 Workflow 的 `retrieve_knowledge` 节点；Wikipedia 改为 ReAct 子图中的 Tool。完整当前架构见 **[后端整体架构图](./后端整体架构图.md)**。
...
> 以下为 V1 历史架构。当前实现见 [后端整体架构图 §1–2](./后端整体架构图.md)。

#### 2.2.0 当前架构（2026-06，Phase 3 LangGraph Agent）
...
ChatStreamOrchestrator
  ▼
LangGraphAgentRunner → AstroGuideWorkflowRunner
  route → retrieve_knowledge → [plan] → prepare_context
  → react_agent (ReAct) → review_answer → finalize
  ▼
MySQL + Qdrant + LLM API + Wikipedia API (via Tool)
```
...
> **实现状态（2026-06）**：本节描述的 Agent 演进目标已实现。当前采用 LangGraph4j Workflow + ReAct 子图；RAG 在 `retrieve_knowledge` 节点执行；Wikipedia/KB/ConceptCard 为 `@Tool`，由 ReAct 循环按需调用。详见 [后端整体架构图](./后端整体架构图.md) 与 [V1.5 Agent 文档](./AstroGuide-V1.5-技术设计文档（Agent）.md)。

本节描述从「固定管线」演进为「由 LLM 按需调用工具」的 Agent 形态时的设计原则：什么适合保持为 **Advisor**（由我们在调用 LLM 前注入的上下文），什么适合变成 **Tool**（由模型在推理过程中主动调用的能力）。V1 实现为固定管线；以下为 V1.5+ 演进方向（**已实现**）。
