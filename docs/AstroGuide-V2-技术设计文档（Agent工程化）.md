...
- Spring AI `@Tool` 工具调用（Wikipedia / KnowledgeBase / ConceptCard）
- RAG（VectorStore + QuestionAnswerAdvisor）
- ChatMemory 注入、基础限流与 usage 统计

但离“严肃 Agent 产品”仍有关键差距：
1. **执行可恢复性不足**：一次请求失败后缺少标准化恢复与重放机制。
2. **过程可观测不足**：tool_call/tool_result 过程主要在框架内部，缺少 span 级链路追踪与统一审计。
3. **质量闭环不足**：缺少离线评测集、回归基线、线上反馈回流与失败样本沉淀。
4. **治理与边界不足**：工具预算、超时分层、数据边界、注入防护与降级策略未系统化。
5. **长链路能力不足**：目前偏“问答型 agent”，不支持任务级状态机、异步任务与子 agent 委派。
...
API Layer
  ...
  └─ SSE Protocol (meta/delta/done/error [+optional events])

Orchestration Layer
  ├─ ChatStreamOrchestrator (现有)
  ├─ AgentExecutionCoordinator (新增)
  ├─ Policy Chain (限流/预算/超时/降级/安全)
  └─ AgentRunStore (执行快照/重放)

Intelligence Layer
  ├─ ChatStreamServiceImpl (现有, 继续作为 Spring AI 入口)
  ├─ Tool Registry & Capability Gating (新增)
  ├─ Memory Pipeline (短期记忆 + 长期摘要)
  └─ Evaluation Hooks (离线/在线评测埋点)

Infrastructure Layer
  ├─ MySQL (会话/消息/任务/审计)
  ├─ VectorStore(Qdrant)
  ├─ Redis (配额/缓存/分布式状态，按阶段引入)
  ├─ Metrics/Tracing/Logging (Micrometer + OTel)
  └─ Async Executor (TaskExecutor/CompletableFuture)
```
